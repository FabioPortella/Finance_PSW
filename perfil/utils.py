...
def calcula_equilibrio_financeiro():
    gastos_essenciais = Valores.objects.filter(
        ...
        tipo = 'S').filter(
        categoria__essencial = True)  # pega da tabela categoria, no campo essencial
    
    gastos_nao_essenciais = Valores.objects.filter(
        data__month = datetime.now().month).filter(
        tipo = 'S').filter(
        categoria__essencial = False)  # pega da tabela categoria, no campo essencial
    ...
    # TODO: corrigir erro de lógica - quando somente um dos percentuais for zero
    try:
